...
struct Evaluation {
    ...
    int value(int id,int changed_tag)const {
        const auto& n=f.nodes[id];
        if(n.kind=='x' || n.kind=='t')return 0;
        if(n.kind=='n')return (1-value(n.children[0],changed_tag)+p)%p;
        if(n.kind=='o') {
            if(tags[id]!=changed_tag)return 1;
            ...
            return value(children[0],changed_tag);
        }
        int scalar=n.residue-int(n.children.size());
        for(int child:n.children)scalar+=value(child,changed_tag);
            ...
            for(size_t j=0;j<from.size();j++) {
                ...
                if(!literal) {
                    ...
                    for(int tag:eval.supports[to[j]])if(cuts[tag] && live[tag] && !eval.supports[from[j]].count(tag)) {
                        int x=eval.value(from[j],tag),y=eval.value(to[j],tag);
                        if(x==y)continue;
                        out<<",\"changed_future_family\":"<<tag<<",\"source_input_value\":"<<(1-x+p)%p
                           <<",\"target_input_value\":"<<(1-y+p)%p;found=true;break;
                    }
...
int main(int argc,char** argv) {
    try {
        ...
        out<<"{\"schema\":1,\"suite\":\"private_mod_frontiers\",\"seed\":null,"
               ...
               "\"nonidentity_model\":\"all proposition values and coefficients zero except the first coefficient of the named family, which is one; companion equations are not imposed\"}\n";
