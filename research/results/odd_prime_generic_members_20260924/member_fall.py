"""Hypothesis (c) of relative gluing over the weak base, in the weak-base stability range: does one generic member fall?

Tested statement: the homogenized weak unary base (r rows, N holes; Booleanity, column exclusions, row equations) plus
T generic selectors 1 - (l_t - v_t)^2 = 0, l_t = sum_ij f_ij x_ij uniform random row-dependent forms, is z-injective
below D: dim ker(z: B_k -> B_{k+1}) = HF_B(k) + HF_A(k+1) - HF_B(k+1) = 0 for k < D (A = B/zB), from exact Groebner
Hilbert functions over F_3 truncated at D (Singular).  With Z the whole slice the closed base is the weak base, so this
is (c) over the closed base in that case.  In range: N >= (p-1)D^2 and r >= D (thm:weak-base-stability-quadratic).
Positive control: the unclosed column merge C_1 C_2, which falls on small boards (ex:placed-pigeon-falls).
Usage: python3 member_fall.py OUT.json r:N:D:seed:member   (member: base, gsel1, gsel2, gsel4, merge)"""
