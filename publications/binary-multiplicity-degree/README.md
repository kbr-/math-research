...
The least degree D(n,k) of a polynomial over F₂ in n variables with Hasse multiplicity at least k
at every nonzero point and less than k at the origin is 2k + n − 2 − ⌊log₂ k⌋ for k < 2^(n−1), and
the Schwartz–Zippel bound 2k − ⌊k/2^(n−1)⌋ for k ≥ 2^(n−1)
([Lean](https://github.com/kbr-/math-research/blob/8651228097422970d147972353bbf9a1c6ed6938/formalization/claims/BinaryMultiplicityDegreeComplete.lean)).
The bound, asked for by Bishnoi, Boyadzhiyska, Das and Mészáros, is attained if and only if
k ≥ 2^(n−2); above that, their hyperplane covers attain it. The new range is k < 2^(n−2).
...
valid in every dimension. Menezes determines the per-order minimum for n ≥ k − 1; with the
hyperplane covers, this paper adds the range ⌊log₂ k⌋ + 3 ≤ n < k − 1.

Every statement links to its Lean source at commit `86512280`. The two external inputs, the
Schwartz–Zippel lemma and the hyperplane covers, are stated without proof in the text, but they are
formalized as well, so the formal verification has no unchecked premise. The paper supersedes the deduction of the earlier
...
~~~sh
./formalization/verify.sh --target claims/BinaryMultiplicityDegreeComplete.lean
~~~

The expected axiom report for the main theorem is

~~~
'MathResearch.BinaryMultiplicityDegreeComplete.degree_complete' depends on axioms: [propext, Classical.choice, Quot.sound]
~~~
