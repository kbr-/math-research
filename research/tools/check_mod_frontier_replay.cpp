...
void fixture(std::ostream& out,int id,int p,int width,int h,int count,bool multiple,bool direct_only) {
...
int main(int argc,char** argv) {
    try {
        need((argc==3 || argc==6) && std::string(argv[1])=="--out","--out NEW_PATH [--case ID --direct-only] required");
        int selected=-1;bool direct_only=false;
        if(argc==6) {
            need(std::string(argv[3])=="--case" && std::string(argv[5])=="--direct-only","unknown selection flags");
            selected=std::stoi(argv[4]);need(selected>=0 && selected<12,"case ID must be 0--11");direct_only=true;
        }
        ...
        out<<"{\"schema\":1,\"suite\":\"mod_frontier_replay\",\"seed\":null,\"requested_case\":"<<selected
           <<",\"direct_only\":"<<(direct_only?"true":"false")<<"}\n";
        int id=0,ran=0;
        auto run=[&](int p,int width,int h,int count,bool multiple) {
            if(selected<0 || selected==id) {fixture(out,id,p,width,h,count,multiple,direct_only);ran++;}
            id++;
        };
        for(int p:{2,3}) {run(p,3,1,1,false);run(p,5,1,1,false);run(p,2,2,1,false);}
        for(int k:{3,5})run(2,2,1,k,true);
        for(int k:{2,4})run(3,2,1,k,true);
        for(int k:{2,3})run(5,2,1,k,true);
        controls(out);
        out<<"{\"record\":\"summary\",\"cases\":"<<ran<<",\"pc_traces\":"<<ran*(direct_only?4:8)
           <<",\"scope_controls\":8,\"all_passed\":true}\n";
        out.close();need(bool(out),"output write failed");
        std::cout<<ran<<" selected cases and "<<ran*(direct_only?4:8)<<" PC traces passed with eight scope controls.\n";
