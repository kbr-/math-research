...
This appendix gives a second proof of Corollary~\ref{cor:main}, the one of
...
upper bound is Proposition~\ref{prop:menezes-polynomial} for \(k<2^{n-1}\) and
the hyperplane covers (Remark~\ref{rem:hyperplane}) for \(k\ge2^{n-1}\).
...
Let \(c=\lfloor k/M\rfloor\), and suppose \(d+c<2k\). Then
\(dM+(c+1)M\le2kM\). Since \((c+1)M>k\), this gives \(dM<2kM-k=k(2M-1)\), a
contradiction.
\end{proof}
...
\begin{proof}
The shift of \(Q(c,x')\) at \(a'\) is \(Q((c,a')+(z_0,z'))\) with \(z_0=0\). Its
coefficients are the coefficients of \(Q((c,a')+(z_0,z'))\) at monomials without
\(z_0\), and these have the same degree.
\end{proof}
...
\begin{itemize}
 ...
 Take a monomial \((x')^\beta\) with a nonzero coefficient in it. Then
 \(|\beta|=\ell\), and the same coefficient appears in \(Q(0,x')\). The
 coefficient of \((x')^\beta\) in \(Q(1,x')\) vanishes, because
 \(\ell<k\le\mult_{\zero}(Q(1,x'))\). So \((x')^\beta\) occurs in \(S\), and
 \(\mult_{\zero}(S)\le\ell\). In particular \(S\ne0\).
 \item \emph{Degree.} Since \(S\ne0\) and \(\deg Q_t+t\le\deg Q\) for every
 \(t\ge1\), \(\deg S\le\deg Q-1\).
\end{itemize}
Finally \(\deg S+1\le\deg Q\le\deg P\).
\end{proof}

\begin{remark}\label{rem:dimension-step}
In particular \(D(n-1,k)\le D(n,k)-1\) for \(n\ge2\) and \(k<2^n\), and, since
the origin order is kept exactly, the same holds for \(\delta\). By
Corollary~\ref{cor:main} and Theorem~\ref{thm:main}, both inequalities hold for
every \(k\ge1\)
\textnormal{\small\leanref{claims/DegreeFormulaArithmetic.lean}{[Lean: \(D\)]}\,\leanref{claims/PerOrderArithmetic.lean}{[Lean: \(\delta\)]}}.
\end{remark}
...
\begin{proof}
Let \(n_0=\lfloor\log_2k\rfloor+2\), so \(k<2^{n_0-1}\). Argue by induction on \(n\ge n_0\).

For \(n=n_0\), Proposition~\ref{prop:cover-bound} gives
\(\deg P\ge2k-\lfloor k/2^{n_0-1}\rfloor=2k\), which is the claim.

For \(n+1>n_0\), identify the variables with \(\{x_0\}\cup\sigma\), where \(|\sigma|=n\).
...
\(k<2^{n_0-1}\le2^{n+1}\), Theorem~\ref{thm:step} gives a \(k\)-admissible \(S\) in
...
\begin{remark}[Hyperplane covers {\cite[Theorem~1.2(a) with \(d=1\)]{BBDM23}}]\label{rem:hyperplane}
Let \(n\ge1\) and \(k\ge2^{n-1}\), and write \(k=c2^{n-1}+b\) with
\(0\le b<2^{n-1}\). Then \(P=F^c\cdot(x_1^2+x_1)^b\) is \(k\)-admissible and
\(\deg P\le2k-\lfloor k/2^{n-1}\rfloor\)
\lean{third-party-claims/HyperplaneCoverUpperBound.lean}. This is the polynomial
form of a cover of Bishnoi et al.\ by \(c\) copies of the \(2^n-1\) hyperplanes
avoiding the origin and \(b\) copies of the pair \(x_1=0\), \(x_1=1\), and it
follows from Lemma~\ref{lem:hyperplane-product}: the multiplicity at a nonzero
point is at least \(c2^{n-1}+b=k\), the origin order is \(b<k\), and the degree
is at most \(c(2^n-1)+2b=2k-c\).
\end{remark}
...
\end{enumerate}
\textnormal{\small\leanref{claims/BinaryMultiplicityDegreeFormula.lean}{[Lean: parts 1, 2]}\,\leanref{claims/BinaryMultiplicityDegreeComplete.lean}{[Lean: part 3]}}
\end{theorem}
...
Remark~\ref{rem:hyperplane} the upper bound.
\end{proof}
