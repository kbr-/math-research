...
void fixture(std::ostream& out,int id,int p,int width,int h,bool derived_inputs) {
    ...
    auto axioms=domains(p,sizes);std::vector<int> primitive_indices;
    for(int j=0;j<propositions;j++) {
        primitive_indices.push_back(int(axioms.size()));
        if(derived_inputs)axioms.insert(axioms.end(),lower[j].axioms.begin(),lower[j].axioms.end());
    ...
    for(bool negative:{false,true}) {
        int residue=negative?1:0;std::string sign=negative?"negative_residue_one":"positive_residue_zero";
        ...
        Proof source(p,source_axioms);std::vector<int> input_proofs;
        for(int j=0;j<propositions;j++) {
        ...
        need(source.val(sum)==P.product+Q.product-Poly(p,2),"antecedent scalar preparation");
        int source_final=negative?source.lc(source.ax(int(axioms.size())),sum,1,-1):sum;
        for(const auto& line:source.lines)need(!uses(line.value,bar.first,bar.end),"antecedent proof uses the implication copy");
        trace(out,sign+"_antecedent",source,source_final,negative?one:u,s);

        Poly power=powp(v,p-1),alpha=negative?one-power:power,common(p);
        if(negative)common=powp(v,p-2);
        else for(int j=0;j<p-1;j++)common=common+Poly(p,modpow(p-1,j,p))*powp(v,p-2-j);
        ...
        int bound=negative?p*s:std::max(2*s,(p-1)*s);
...
int main(int argc,char** argv) {
    try {
        ...
        controls(out);
        out<<"{\"record\":\"summary\",\"cases\":8,\"pc_traces\":72,\"missing_coverage_controls\":2,\"all_passed\":true}\n";
        out.close();need(bool(out),"output write failed");
        std::cout<<"Eight partial-frontier cases, 72 full PC traces, and coverage controls passed.\n";
