...
One further fact about orders at the origin is used repeatedly. For
\(f\) in a polynomial ring over \(\F\), write \(f_d\) for its homogeneous component
of degree \(d\), and \(\operatorname{ord}f=\mult_{\zero}(f)\) for the least \(d\) with
\(f_d\ne0\). Over \(\Ft\), if \(z_i\) (\(i\in\sigma\)) are elements of
\(\Ft[y_\sigma]\) without constant term, then substituting \(x_i\mapsto z_i\) does
not lower the order: a monomial of degree \(d\) goes to a product of \(d\)
elements of order at least one. Hence \(\mult_a(P)\le\operatorname{ord}P(a+z)\)
for every such substitution \(z\)
