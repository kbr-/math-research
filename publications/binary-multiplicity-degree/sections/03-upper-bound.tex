...
The Catalan number \(C_{a-1}\) is odd exactly when \(a\) is a power of two;
more precisely \(v_2(C_{a-1})=s_2(a)-1\) \cite{AK73}, \cite[Theorem~2.1 of the arXiv
version]{DS06} \lean{third-party-claims/CatalanParity.lean}. So \(g_s\) is the specialization to \(\Ft\) of the Catalan truncation
...
hold for every \(s\ge0\) with natural-number subtraction; only \(s\ge1\) is
used.)
...
\(u\cdot x=\sum_iu_ix_i\). Its \(2^n-1\) factors \lean{claims/PerOrderArithmetic.lean} define the hyperplanes
...
polynomial over \(\Ft\). It works in every dimension, with the digit-sum bound,
also when \(k-\ell-1\ge2^n\), where Theorem~\ref{thm:construction} has degree below
that bound
...
degree exactly \(\Phi(n,k,\ell)\) \lean{claims/PerOrderArithmetic.lean}.
