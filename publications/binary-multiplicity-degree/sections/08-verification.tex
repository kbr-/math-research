...
Substitution, Lemma~\ref{lem:substitution}
& \leanref{claims/PerOrderArithmetic.lean}{\nolinkurl{claims/PerOrderArithmetic.lean}}\newline
\nolinkurl{MR.PerOrderArithmetic.mult_le_mult_zero_subst}\\
...
Catalan parity \cite{AK73,DS06}, Lemma~\ref{lem:catalan}
...
Local inverse, Lemma~\ref{lem:local-inverse}
& \leanref{claims/PerOrderYCoefficient.lean}{\nolinkurl{claims/PerOrderYCoefficient.lean}}
\nolinkurl{MR.PerOrderYCoefficient.xhat_sq_add};\newline
...
Arithmetic of \(\Phi\) and \(\delta\) in the prose: Section~\ref{sec:introduction} (Legendre form,
comparison with Menezes' form, blocks, examples, the top order, the reals, \(n=1\)),
Section~\ref{sec:upper} (exact degree, the factors of \(F\) and of \(F^2\)),
Remark~\ref{rem:dimension-step}
& \leanref{claims/PerOrderArithmetic.lean}{\nolinkurl{claims/PerOrderArithmetic.lean}}\newline
...
Arithmetic of \(D(n,k)\) in the prose: Sections~\ref{sec:introduction} and~\ref{sec:main},
Remark~\ref{rem:dimension-step}; admissible polynomials are nonzero
...
Descent, Lemma~\ref{lem:descent}; characteristic two, Theorem~\ref{thm:char-two}
& \leanref{claims/PerOrderCharTwo.lean}{\nolinkurl{claims/PerOrderCharTwo.lean}}\newline
\nolinkurl{MR.PerOrderCharTwo.coeffMap}, \nolinkurl{coeff_shift_coeffMap},
\nolinkurl{totalDegree_coeffMap_le}, \nolinkurl{lower_bound_char_two},
\nolinkurl{per_order_value_char_two}, \nolinkurl{degree_complete_char_two}\\
...
Hyperplane covers \cite{BBDM23}, Remark~\ref{rem:hyperplane}
...
\begin{itemize}
 ...
 \(t+\Sigma\ge k\).
 \item In \nolinkurl{MR.TruncatedProduct.g} the pair \((S,p)\) of
 Definition~\ref{def:truncated} is encoded by a map
 \(c:\sigma\to\{\bot,0,\ldots,s-1\}\): \(c_i=\bot\) means \(i\notin S\), and \(c_i=j\)
 means \(p_i=2^j\). The sum runs over maps of cost \(\sum_{c_i\ne\bot}2^{c_i}\le
 s-1\), and the bound \(j<s\) is automatic, because \(2^j\le s-1<2^s\). The Lean
 statements hold for every \(s\ge0\) with natural-number subtraction.
 \item The Lean proof of Theorem~\ref{thm:dkss} follows the induction on the
 number of variables in~\cite{DKSS13} (Lemma~8 of the arXiv version). It also
 proves the scaled form \(|S|\sum_a\mult_a(P)\le\deg P\cdot|S|^{|\sigma|}\), which
 holds with no variables.
 \item Lemma~\ref{lem:minimum} has two Lean proofs. The first follows the digit
 argument of Section~\ref{sec:main}. The second derives the minimum from
 Theorem~\ref{thm:main} and the value of \(D(n,k)\) proved in
 Appendix~\ref{sec:second-proof}: every \(\Phi(n,k,\ell)\) is the degree of a
 \(k\)-admissible polynomial, and the origin order of a \(k\)-admissible
 polynomial of least degree attains the minimum.
...
cd bmd-check
git checkout --detach 7fc6175b92fd064c488eca76eba2b04522da7815
cd formalization
...
The target is an import-only module whose header lists 69 declarations: the
...
\end{verbatim}
\endgroup
\noindent from \texttt{formalization/}.
