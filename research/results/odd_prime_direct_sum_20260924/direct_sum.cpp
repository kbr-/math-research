...
// Vanishing polynomials: kernel of evaluation on a random sample of Z_S points; each kernel vector is verified on every
// point of the subcube by a subset-sum (zeta) transform, and failures add points and repeat, so the result is exact.
...
static Block& getBlock(uint32_t S,int e){
  ...
  if(!any){ for(int i=0;i<nm;i++){ Vec v(nm,0); v[i]=1; kerRows.push_back(v);} }
    ...
    while(sample.size()<want&&tries<200000000ull){ uint64_t a=rng()&(n-1); tries++; uint32_t c=lift(a); if(ALW[c]) sample.push_back(c); }
    ...
    while(true){
      ...
      vector<uint32_t> add(kerRows.size(),0); vector<char> bad(kerRows.size(),0);
      #pragma omp parallel for schedule(dynamic)
      for(size_t k=0;k<kerRows.size();k++){
        vector<uint8_t> a(n,0); for(int i=0;i<nm;i++) if(kerRows[k][i]) a[loc[i]]=kerRows[k][i];
      ...
      bool ok=true; for(size_t k=0;k<kerRows.size();k++) if(bad[k]){ ok=false; sample.push_back(add[k]); }
      if(ok) break;
    }
  }
