...
data=load();packets=[deps.metadata_packet(data,c['id'],limit=4,width=1000) for c in data['claims'][15:60]]
...
for packet in packets:
 ...
 for passage in packet['passages']:
  ...
  if snippets:
   print(' ',snippets[0]['text'][:250])
   if len(snippets)>1:print(' ',snippets[-1]['text'][:250])
