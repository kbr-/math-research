...
\(Y\mapsto Y^{2^j}\), the two shifts \(\gamma_i\in\{0,1\}\) in
Lemma~\ref{lem:congruence}, and the telescoping identity. The natural next
questions are the least degree at every origin order over other fields below
Menezes' range \(n\ge k-1\), and the analogue on grids other than
\(\{0,1\}^n\).
\end{remark}
