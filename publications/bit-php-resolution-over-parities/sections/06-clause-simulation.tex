...
In any commutative ring, for a finite input index set and \(h\ge0\), put
\[
 P_v=\prod_{u=0}^{v-1}\left(1-\sum_i r_{ui}g_i\right),\qquad
 U_{h,i}=\sum_{v=0}^{h-1}r_{vi}P_v.
\]
Then \(1-P_h=\sum_i U_{h,i}g_i\).
For ordinary polynomials over any commutative coefficient ring, if
\(\deg g_i\le\delta\), \(\deg r_{vi}\le1\), and \(h\ge1\), then
\[
...
The sum and product degree inequalities prove the three bounds, including
over rings with zero divisors. At \(h=0\), \(P_0=1\) and \(U_{0,i}=0\).
\end{proof}
...
Let \(V\) be any vector space over \(\F\), and \(A,B,D\) finite clauses
...
\]
No finite-dimensional assumption is required.
\end{lemma}
\begin{proof}
...
Over any \(\F\)-vector space, every sound inference \(A\wedge B\models D\)
...
\[
 N=3S+|J|\text{ slots},\qquad W=\max\{w,v+2\},
\]
at most \(NW\) inputs and at most \(v+hNW\) variables. If the supplied
...
\begin{equation}\label{eq:simulation-degree}
 D=\max\{2h+\ell,4h+1\}.
\end{equation}
With \(v=m\ell\), \(N=3S+\binom m2\), and
\(W=\max\{\ell,v+2\}\), the total input count is at most \(NW\),
and the total variable count is at most \(v+hNW\). There is no proof-height restriction.
\end{theorem}
\begin{proof}
