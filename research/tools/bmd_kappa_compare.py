"""Compare exact per-order savings on a grid S^n with the root-curve orders kappa(m).

Statement tested (stable per-order law).  For every finite grid S^n (0 in S) and m = k-l-1, the savings
...
line printed by bmd_root_curve_orders_n.py or bmd_root_curve_orders_grid.py (the first trial with no
COUNT DIFFER; kappa is a specialization, so agreement across trials is the evidence that it is generic) and reports, for each m with a known
...
def main():
    ...
    # use the first trial whose counts all match N_{s,n}(d); a matching count does not prove genericity
    ...
    sav = {}
    for f in kdir.glob('*-n%d-k*.json' % n):
        d = json.loads(f.read_text())
        s, k = (len(d['S']) if 'S' in d else d['grid']), d['k']
