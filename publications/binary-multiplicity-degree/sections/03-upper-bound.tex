...
polynomial over \(\Ft\). Proposition~\ref{prop:menezes-polynomial} is his result
specialized to \(\Ft\), which holds in every dimension \cite[Theorem~3.2 and
Section~8]{Men26}; the proof below uses the telescoping identity instead. When
\(k-\ell-1\ge2^n\) its degree bound exceeds \(\Phi(n,k,\ell)\).
