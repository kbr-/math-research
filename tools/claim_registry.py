...
# Words that assert a boundary (exactness, necessity, optimality, openness). A claim whose summary
# uses one while its Lean coverage is complete must name it in the verified scope too, so that such
# prose cannot ride on a formalization of a weaker statement.
BOUNDARY_WORDS = re.compile(r"\b(exactly|only if|only when|if and only if|iff|never|not attained|"
                            r"optimal|best possible|tight|sharp|open)\b", re.I)
...
def unformalized_boundary_words(claim):
    """Boundary words of a completely formalized claim's summary absent from its verified scope."""
    f = claim['formalization']
    if f['status'] != 'complete':
        return []
    scope = ' '.join([f['scope'] or ''] + [a.get('scope') or '' for a in f.get('artifacts', [])])
    return sorted({w.lower() for w in BOUNDARY_WORDS.findall(claim['summary'])} -
                  {w.lower() for w in BOUNDARY_WORDS.findall(scope)})
...
def validate(data):
    ...
    for c in data['claims']:
        ...
        missing = unformalized_boundary_words(c)
        require(not missing, f"{c['id']}: summary asserts {', '.join(missing)} beyond the complete "
                "formalization scope; formalize and name it in the scope, or narrow the summary")
        if version == 2:
