...
- [ ] Disclose Claude Fable 5.1's role in revision 1, in the same paragraph and consistently with the title-page note: it formalized the exponential corollary and the generic criterion (statements and proofs, with one proof batch delegated to Claude Opus 5), triaged this feedback, and will draft the revised text. The author again did not check the mathematics and relies on Lean. Update the README line "only been tested with GPT-6 Astra" at the same time.
