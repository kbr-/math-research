"""With an optional resolvent clause R (forbids all private forms nonzero, the resolution of an inconsistent
cluster): does adding R restore locality?  Degreewise locality for budget-type clauses in form space over F_3: each clause forbids 'all its forms nonzero'
...
Usage: python3 resolvent_absorption.py OUT.json t:k_1,...,k_{t+1}:c:R [...]   (R = 1 adds the resolvent)"""
import itertools, json, sys, time
import numpy as np
exec(open(__file__.replace('odd_prime_cluster_avoidance_20260924/resolvent_absorption.py',
                           'odd_prime_t_wise_locality_20260924/form_locality.py')).read().split('def indep_t')[0])
out = sys.argv[1]; res = []
for case in sys.argv[2:]:
    t, ks, c, addR = case.split(':'); t = int(t); addR = int(addR); ks = [int(x) for x in ks.split(',')]; c = int(c); t0 = time.time()
    wpos = []; pos = t
    ...
    for b in range(t + 1):
        ...
        forb.append(ynz & wnz)
    allw = [j for wp in wpos for j in wp]
    if addR: forb.append(np.all(pts[:, allw] != 0, axis=1))
