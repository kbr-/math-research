...
struct Arithmetic {
    ...
    void add(EP& a,const EP& b,int scalar=1) const{
        a.resize(f.d);need(b.size()==unsigned(f.d),"extension coordinate count");
        for(int i=0;i<f.d;++i){
    ...
    EP substitute(const EP& p,const std::map<int,EP>& images) const{
        ...
        for(int i=0;i<f.d;++i)for(const auto& [m,c]:p[i]){
            need(c==1,"binary component coefficient");auto term=constant(1<<i);
    ...
    void write(std::ostream& out,const EP& p) const{
        Polynomial wire;
        for(int i=0;i<f.d;++i)for(const auto& [m,c]:p[i]){need(c==1,"wire coefficient");wire[m]^=1<<i;}
        write_json(out,wire);
    }
};
using Matrix=std::vector<std::vector<int>>;
Matrix invert(const SmallField& f,Matrix matrix){
    int n=matrix.size();for(int i=0;i<n;++i){matrix[i].resize(2*n);matrix[i][n+i]=1;}
    for(int col=0;col<n;++col){
        int pivot=col;while(pivot<n && !matrix[pivot][col])++pivot;need(pivot<n,"Moore matrix invertibility");
    ...
    for(int i=0;i<n;++i)for(int j=0;j<n;++j){need(matrix[i][j]==int(i==j),"matrix left identity");result[i][j]=matrix[i][n+j];}
    return result;
}
void matrix_json(std::ostream& out,const Matrix& m){
