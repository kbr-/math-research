...
static void run_case(std::ostream& o,const std::string& name,const Source& s,int D,bool& all_ok){
  ...
  for(size_t i=0;i<news.size();++i){ const Poly& t=news[i];
    ...
    int enters=-1; for(int E=D+1;E<=s.nvars;++E){ Space b(s,E,false); if(Space::lead(b.reduce(b.vec(t)))<0){ enters=E; break; } }
...
int main(int argc,char** argv){
  if(argc!=3||std::string(argv[1])!="--out"){ std::cerr<<"usage: --out PATH\n"; return 2; }
