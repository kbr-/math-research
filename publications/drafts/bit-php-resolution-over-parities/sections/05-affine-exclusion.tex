...
\lean{claims/AffineSystemLinearAlgebra.lean}
\lean{claims/FiniteAffineSpan.lean}
\lean{claims/FiniteAffineCoordinates.lean}
Over any field \(K\), let a finite affine system on \(K^v\) have span \(S\).
...
\lean{claims/FreshENSScalarCleanup.lean}\lean{claims/FreshENSBlock.lean}
...
More generally over any field, such a substitution works whenever the chosen
constants satisfy their Boolean equations and every eliminated companion
maps to zero. If a block has a genuine degree-one input, then
...
\lean{claims/OrdinaryRestrictionDimension.lean}
\lean{claims/AffineRestrictionIdeal.lean}
Over any field, an affine substitution into \(d\) variables maps a space
...
\lean{claims/RestrictionKernelBounds.lean}
\lean{claims/CommonAffineRestrictionKernel.lean}
...
\]
There is no multilinearity or properness requirement. The block family
itself may be arbitrary; a PC derivation uses only finitely many axioms.
\end{lemma}
\begin{proof}
