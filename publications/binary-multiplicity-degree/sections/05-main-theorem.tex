...
\textnormal{\small\leanref{claims/PerOrderMinimumDirect.lean}{[Lean: this proof]}\,\leanref{claims/PerOrderMinimum.lean}{[Lean: via Section~\ref{sec:second-proof}]}}.
\end{lemma}
...
The first Lean proof of Lemma~\ref{lem:minimum} follows this digit argument. A
second one derives the minimum from Theorem~\ref{thm:main} and the value of
...
\(D(n,k)=2k-3\) \lean{claims/DegreeFormulaArithmetic.lean}.
...
\(\ell=k-2^{\lfloor\log_2k\rfloor}\) \lean{claims/PerOrderMinimumDirect.lean}. Exact rank computations of
